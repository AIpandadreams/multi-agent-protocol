#!/usr/bin/env python3
"""Mirror-consistency CI for the agent-protocol skills (PROTOCOL v3.2).
...
def stamped(p: Path) -> bool:
    ...
    return "v3.2" in text(p) or p.relative_to(ROOT).as_posix() in STAMP_EXEMPT
...
# 5. Version stamp coverage: every reference/skill file carries v3.2 — unless the
...
for p in SKILLS.rglob("*.md"):
    check(stamped(p), f"missing PROTOCOL v3.2 stamp: {p.relative_to(ROOT)}")
...
# skill tree does: a v3.2 stamp, no duplicated core rule blocks, and the same
...
if TRANSPORTS.is_dir():
    for p in TRANSPORTS.rglob("*.md"):
        t = text(p)
        check(stamped(p),
              f"missing PROTOCOL v3.2 stamp: {p.relative_to(ROOT)}")
        for h in CORE_HEADINGS:
            check(h not in t,
                  f"core block '{h}' duplicated in {p.relative_to(ROOT)}")
        for pat, why in BANNED:
