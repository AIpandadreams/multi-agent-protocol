#!/usr/bin/env python3
"""Mirror-consistency CI for the agent-protocol skills (PROTOCOL v2.9).
...
def stamped(p: Path) -> bool:
    ...
    return "v2.9" in text(p) or p.relative_to(ROOT).as_posix() in STAMP_EXEMPT
...
# 5. Version stamp coverage: every reference/skill file carries v2.9 — unless the
...
for p in SKILLS.rglob("*.md"):
    check(stamped(p), f"missing PROTOCOL v2.9 stamp: {p.relative_to(ROOT)}")
...
# skill tree does: a v2.9 stamp, no duplicated core rule blocks, and the same
...
if TRANSPORTS.is_dir():
    for p in TRANSPORTS.rglob("*.md"):
        t = text(p)
        check(stamped(p),
              f"missing PROTOCOL v2.9 stamp: {p.relative_to(ROOT)}")
        for h in CORE_HEADINGS:
            check(h not in t,
                  f"core block '{h}' duplicated in {p.relative_to(ROOT)}")
        for pat, why in BANNED:
