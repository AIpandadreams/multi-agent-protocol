...
def stamped(p: Path) -> bool:
    ...
    return "v2.6" in text(p) or p.relative_to(ROOT).as_posix() in STAMP_EXEMPT
...
# 5. Version stamp coverage: every reference/skill file carries v2.6 — unless the
...
for p in SKILLS.rglob("*.md"):
    check(stamped(p), f"missing PROTOCOL v2.6 stamp: {p.relative_to(ROOT)}")
