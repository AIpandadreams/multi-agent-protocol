#!/usr/bin/env python3
"""Migrate a stamped workspace up to PROTOCOL v3.2 [PROTOCOL v3.2].
...
WHAT IT DOES (mechanical, idempotent, reversible):
  ...
  - EXCEPT append-only records: `memory/<role>/auth-log.md`,
    ...
    EVERY hop of a chained run: a v2.5-era record survives a v2.5 -> v3.2
...
stays green under a v3.2 checkout, so workspaces migrate independently, each at
...
Idempotent: a second run finds v3.2 and does nothing. Exit 0 = migrated /
already-v3.2 / dry-run; 1 = the pin is not a supported starting version
...
def migrate(ws, dry_run=False):
    ...
    # v2.5 -> v3.2 is reported for the v2.5 token it keeps.
