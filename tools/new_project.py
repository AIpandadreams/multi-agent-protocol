#!/usr/bin/env python3
"""Stamp a dedicated per-project agent workspace (PROTOCOL v2.9).
...
BINDINGS_TEMPLATE = """# BINDINGS — {name} agent workspace [PROTOCOL v2.9]
...
"""Auth-log chain validator [PROTOCOL v2.9] — mechanical enforcement of the
...
def stamp(name, dest, profile, roles, role_side, principal,
    ...
    (dest / "channel" / "INDEX.md").write_text(
        f"# REVIEW-ROUND LEDGER — {name} [PROTOCOL v2.9]\n\n"
    ...
    for role in roles:
        ...
        (dest / "memory" / role / "auth-log.md").write_text(
            f"# AUTH-RECORD — {name} / {role} [PROTOCOL v2.9]\n\n"
