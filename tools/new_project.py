#!/usr/bin/env python3
"""Stamp a dedicated per-project agent workspace (PROTOCOL v3.1).
...
BINDINGS_TEMPLATE = """# BINDINGS — {name} agent workspace [PROTOCOL v3.1]
...
"""Auth-log chain validator [PROTOCOL v3.1] — mechanical enforcement of the
...
def stamp(name, dest, profile, roles, role_side, principal,
    ...
    (dest / "channel" / "INDEX.md").write_text(
        f"# REVIEW-ROUND LEDGER — {name} [PROTOCOL v3.1]\n\n"
    ...
    for role in roles:
        ...
        (dest / "memory" / role / "auth-log.md").write_text(
            f"# AUTH-RECORD — {name} / {role} [PROTOCOL v3.1]\n\n"
