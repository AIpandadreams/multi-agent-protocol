...
- **ships `tools/render_head.py`** (1,469 lines) with `tests/test_render_head_tails_stamp.py`
  (576 lines, 12 test methods) — it derives a role file's working-state head from
  the open `plans/*.plan.yaml` ledger and the channel lane tails, and stamps an
...
- **adds two SOP-registry gates to `tools/mirror_check.py`** (ids 14b and 14c,
  ...
  SOP count, the SOP names, and their order against the rule catalog in
  `docs/CREATOR-SEAT-BOOTSTRAP.md`, so a registry that describes itself wrongly fails
  the mirror check instead of shipping. The comparison is across the two documents,
  not within the registry: a registry checked only against its own table would agree
  with itself while both drifted;
- **makes `tools/new_project.py` stamp a non-mintable id placeholder** — a seeded
  workspace can no longer be mistaken for one carrying live fleet identifiers. One
  emitted string, one call site; nine of the ten added lines are the comment that
  explains why the placeholder must stay unmintable;
- **installs PyYAML in CI before the suite runs** — the ledger tooling's single
  non-stdlib dependency was already declared but not installed in the workflow;
- **renames the four positions of a converging review from "seats" to "stages"** —
  fourteen word-instances on eleven changed lines across six files, covering the
  `## The four seats` heading, the four pointers to it, and the `CORE_HEADINGS`
  guard entry in `tools/mirror_check.py` that named it. (Both figures are stated
  because they differ: eleven is the changed-line count, and a line can carry more
  than one instance — `review-convergence.md` alone carries nine.) The word collided with the workspace's own
...
- **narrows the "no network calls" trust claim** at its two sites (`README.md`,
  `docs/FAQ.md`) and enumerates what actually leaves the machine — an absolute
  claim that a reader could falsify is worse than a bounded one;
- **corrects `tools/compaction_inject.py`'s `--ack` docstring**, which described a
...
- **cures four defects found inside this delta**, by auditing the release against
  ...
  **vacuous**, which is exactly why it stayed green, and the protection it had been
  ...
  latter. (2) `CORE_HEADINGS` in `tools/mirror_check.py` still listed `## The four
  seats` after the heading was renamed. That guard asserts **absence**, so the stale
  entry could only ever have fired on the obsolete literal — a string the rename had
  just removed from the tree. It was **green while protecting nothing that still
  existed**, and a role file could have duplicated the four-stages block undetected.
  (Not "could not fail": it could, on `## The four seats`. It could not protect.) This is the only cure with
  executable effect, and it restores protection rather than fixing a word —
...
**Corrections to dated claims.** `docs/PLAN-LEDGER.md`'s status table is corrected
to record that wake integration shipped **as of 1.9.0** — a retroactive correction
to the table, not something shipping now. The protected-paths material added here
**documents an existing CI check**; it adds no check.

**Self-dating language.** Seven "in this release" phrases were removed or dated
across three files, because a phrase that means something different in every later
release is a defect the moment the release lands. ⚠ The sweep is not complete and
...
**Scope correction.** 1.9.0 deferred two artifacts its own authorizing scope decision
had placed in the shipping set: the compaction re-injection hook and the `plans/`
operator README. The deferral honored a reviewed release sentence ("ships no new
tooling") at the cost of the principal's recorded word; surfaced, the principal ruled
the scope word governs. This release:

- **ships `tools/compaction_inject.py`** — part 2 (the memory failsafe) of
  `docs/PLAN-LEDGER.md`, as runnable tooling (PreCompact `--mark`, SessionStart
  `--inject`; prerequisite PyYAML, the single non-stdlib dependency, stated in
  `plans/README.md`);
- **ships `tools/plan_common.py`** beside it — the shared ledger-access module the
  hook imports. The originating enumeration named the hook alone; a hook that cannot
  import is not shipped, so the dependency ships and is named rather than smuggled;
- **creates `plans/`** with `plans/README.md`, the operator-facing seed. No schema
  file and no example plan file ship — that much of 1.9.0's sentence stays true;
- **updates `docs/PLAN-LEDGER.md`'s status table** to match the tree (part 2 shipped;
  parts 3–5 still specified-and-deferred; part 1 seeded);
- notes the incompleteness honestly: the hook's `--ack` discharge path completes only
  when the enforcement gate (part 3) lands with its own reviewed release.

The public copies are derived from the private originals: internal review-round
citations removed from comments and from five emitted stderr/digest strings (each
surrounding sentence kept where it survives losing its citation); the post-injection
hint reworded so it no longer instructs the operator to run a file that does not
ship (the enforcement gate's filename remains, named as a future release); and one deliberate
behavior hardening — the PyYAML import is guarded so a clone without it degrades to
a loud ledger-unreadable error under the declared fail direction rather than crashing
a session-start hook. `python -m py_compile` clean; ship-time content scrub and
fleet-identifier gate runs recorded in the maintainers' release workpapers, the two
residual id-shaped tokens adjudicated as public technical forms.

Process: adopted with this release, normal releases land via the PR-flow convention
(branch → PR → merge, no approval quorum; this release itself arrives by that route) —
...
## [1.9.0] — 2026-08-07 [UNCUT — rolled into 1.9.1]
...
format every other release carries; nothing is new text. **1.9.0 was never
independently cut**: it carries no git tag and no Release object, and the
five-surface release runbook was completed for 1.9.1 only — 1.9.0 is the merged
intermediate the 1.9.1 cut absorbed, not a version an adopter can check out by
tag. The section is kept because the merge is real and this is its reviewed
record.)
...
The bundled workspace-lifecycle tooling crosses in the same commit, so no
checkout is ever internally skewed: `migrate_workspace.py` gains the
**v2.8 → v2.9** hop *beside* the ones it already carries — a v2.5 workspace
now walks FOUR hops in one run of one checkout — `conformance_check.py`
accepts v2.9 while keeping v2.5 through v2.8 green so older workspaces stay
conformant under a newer checkout, `new_project.py` stamps fresh workspaces
v2.9, and `mirror_check.py` gates on the new stamp. There are **no new binding
slots**.
...
- **`CONTRIBUTING.md` — what a green scrub does not prove.** The scrub gate CI
  runs points at the example pattern list, whose organization-specific entries
...
- **`CONTRIBUTING.md` — how merges land.** The file gains this
  entry: "Merges to `main` land as **merge commits**. Squash and rebase merges
  are disabled on this repository on purpose: a landing here is approved
  against one specific reviewed commit, and a squash would land a new commit
  that nobody reviewed."

## [1.5.0] — 2026-07-24

**`PROTOCOL v2.8` — the self-improvement protocol, dogfooded.** Every change
in this release was proposed, reviewed under the independent cross-vendor +
isolated-judge convergence the protocol prescribes, and adopted by the
principal through the same Observe → Propose → Review → Adopt loop
[AUTONOMY.md](docs/AUTONOMY.md) describes — the protocol amending itself by
its own rules. Eight clauses tighten the loop across its span — proposal-time
observability, what a review round accepts as evidence, the amendment hard
rails, how a recommendation travels at a decision of record, and when a worker
may stay idle; a standing package (R1-R10) hardens the instruments that do the
checking. There are **no new binding slots**, and the bundled
workspace-lifecycle tooling crosses in step: `migrate_workspace.py` gains the
**v2.7 → v2.8** hop beside the ones it already carries, `conformance_check.py`
accepts v2.8, and `new_project.py` stamps fresh workspaces v2.8 — all in the
single atomic version commit, so a checkout is never internally skewed.

The eight clauses:

- **Negative-evidence admissibility** (review step) — an absence-claim used as
  gate evidence (that something is clean, zero, or not there) is admissible
  only when it names the instrument that looked and that instrument's known
  blind spots; a positively-phrased *all-present* claim is an absence-claim
  too. Silence is not proof.
- **Observability guarantee** — every mechanism that makes the work easier to
  run (a monitor, a summary, a gate) also makes some failure harder to see; it
  is responsible only if it declares what it makes harder to observe and ships
  the compensating probe.
- **Provenance-admissibility** (hard rails) — an artifact offered as gate
  evidence (a table, count, hash, diff, or measurement) is inadmissible unless
  it carries the verbatim command or procedure that produced it, the inputs
  it was applied to, and — where it asserts a count, shape, or coverage — the
  assertion checked when it was written, all recorded so a reader can
  re-derive it rather than trust it; citations of authority are governed
  identically — authority asserted in prose, resolving to no readable entry,
  is not a citation.
- **KPI authorship and pairing** — the metrics that *gate* whether an amendment
  merges are the principal's, authored or ratified; and within that gating set
  a velocity metric is admitted only paired with its corresponding
  defect-escape metric.
- **Dry-dock** — a worker may bound its attention to the queued units of work
  and stay idle when the queue is empty; this sits directly on the standing
  idle-is-correct doctrine rather than beside it as a tool.
- **Draft-time gate-touch tripwire** (hard rails) — a scheduled or automated
  drafter checks its own diff, before filing, against a principal-authored
  manifest of protected regions; a draft that touches one is not filed as a PR
  but downgrades to a notice-only memo, leaving the locked-region change to the
  principal.
- **Counter-case** — a recommendation offered at a decision of record travels
  with the strongest case against itself.
- **Evidence form** — a claim offered as evidence in a review is admissible
  only as a mechanical *extraction* (content a named instrument takes from a
  stated substrate), never composed about it; reasoning, severity, and
  recommendation stay free prose, and the bar reaches only what is offered as
  grounds.

**R1-R10 — instrument discipline.** A standing package folded into the review
and verification surfaces, each rule earned by a real miss: print the
denominator beside a verdict; a count names the domain it counts over; a
header self-labels what it governs; a pin proves identity, not just presence;
a cure is swept for the very defect it cures; the premises a text-only judge
cannot check are enumerated in its verdict; and the rest — carried now where
the work happens rather than in a lessons file no gate reads.
...
> **Open amendment note (edit made 2026-08-07, recorded 2026-08-20):** this
> published section was edited in place after the v1.2.1 tag was cut — the
> incident-case-study count in the `CREATOR-SEAT-BOOTSTRAP` bullet below
> originally read "nine" and was corrected to "eleven" during the 1.9.0
> documentation release (which discloses the recount in its own entry). The
> in-place method contradicted the house rule the 1.9.1 entry states — *"amends
> the sentence openly rather than editing published history"* — so the edit is
> recorded openly here, at the site, rather than left silent. The corrected
> number stands; the published text of this section is otherwise unedited.
