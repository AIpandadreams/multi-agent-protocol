...
**`PROTOCOL v2.9`.** The three entries below are not notes about how this
repository happens to work — each states a rule that binds a seat: what a
release must be reconciled against before it can be called complete, what a
gate's green is allowed to claim, and what a standing pre-authorization does
and does not reach. New normative rules are a protocol change here, so the
stamp crosses to v2.9 rather than riding a documentation release at v2.8.
