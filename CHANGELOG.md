...
- **adds two SOP-registry gates to `tools/mirror_check.py`** (ids 14b and 14c,
  ...
  registry that describes itself wrongly fails the mirror check instead of shipping;
...
- **renames the four positions of a converging review from "seats" to "stages"** —
  eleven word-instances across six files, covering the `## The four seats` heading,
...
- **cures four defects found inside this delta**, by auditing the release against
  ...
  **vacuous**, which is exactly why it stayed green, and the protection it had been
  ...
  seats` after the heading was renamed. That guard asserts **absence**, so a stale
  entry cannot fail: it was **green while protecting nothing**, and a role file
  could have duplicated the four-stages block undetected. This is the only cure with
