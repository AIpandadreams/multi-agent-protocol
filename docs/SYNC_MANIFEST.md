...
plugin tree is declared **canonical**. **BIG-3 Axis-1 EXECUTED 2026-07-10** (on
...
|---|---|---|
| `plugins/agent-protocol/**` (skill tree) | `plugins/agent-protocol/**` | **ADOPTED @ public v1.2.0 (BIG-3 Axis-1, 2026-07-10)**: the 33 public-ahead files overlaid + 3 public-only files added (`commands/converge.md`, `references/never-idle-core.md`, `references/review-convergence.md`), LF-normalized to this tree's `i/lf w/lf` convention. Private-only deltas re-applied on top (the de-genericization set enumerated above); `plugin.json` versioning stays the private scheme (`2.7.0` as of the 2.6.1→2.7.0 bump recorded above; schemes still differ — public `plugin.json` tracks the public release number, `1.4.0`). |
...
- `plugins/agent-protocol/skills/overnight-mode/SKILL.md` — a **byte-identical
  ...
  byte-custody guarantee vs his live copy. Ruled a permanent **record-exception,
...
2. **Classify each divergence** — *public-ahead* (a back-port candidate, gated on
   BIG-3 timing) · *private-only* (either push the genericized form up to public,
...
so the 3 BOMs the gate flagged in this tree's untracked `docs/big3/*.ps1` are
**false positives** and those files are correct as-is. The mojibake it would cause
...
`agent-core/references/never-idle-core.md` **byte-identical** to public
(blob `a12256f8`: three-state ledger + anti-invention clamp + gate-preserving
invariant), and `agent-core/SKILL.md` **overlay-preserving hand-port** of the
