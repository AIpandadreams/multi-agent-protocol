...
*Currency: the current protocol version is v3.2; the mechanisms in this
document have not been re-verified against v3.2.*
...
> release 1.11.0. When the tree changes, regenerate this block (and its twin —
...
1. **The main commit.** Land the stamp — which is wider than the two
   ...
   The table's SECOND cell is a THIRD surface inside this one file, and it is
   not the release number: it names the PROTOCOL stamp the release carries, so
   it moves only on a restamp — and when it does, it moves HERE, in the release
   that ships it. Omitting it is how the repository came to advertise a protocol
   version its own map of record had never heard of; mirror_check.py section 15
   now fails on exactly that, so the omission is a red gate rather than a
   reader's later discovery.
