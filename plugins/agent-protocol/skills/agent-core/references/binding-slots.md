...
| ROLE_ALIASES | OPTIONAL display-name map: `<display>→<canonical role>` comma-separated (e.g. `engine→owner, helper→builder`) | every `<display>` MUST be one of the bound SIDE_NAMES; targets are canonical roles only; charset `[A-Za-z0-9-]+` — UNDERSCORE FORBIDDEN (it breaks the `<from>_to_<to>_<date>` channel filename grammar); absent row = side names equal the canonical role names. Aliases affect ADDRESSING and display ONLY — ROLE_LOCK, `memory/<role>/` paths, and START_SESSION files always use the canonical role |
