...
# The Owner / Engine Agent — PROTOCOL v2.8
...
**Protocol version:** this skill implements PROTOCOL v2.8. Channel entries carry
the `[v2.8]` stamp; a version mismatch with the peer is flagged and parks
...
| CHANNEL | the channel transport instance (the workspace repo's `channel/`, bound to a shared filesystem or a git remote per the TRANSPORT slot — see `transports/`) + the per-direction files: your append-only outbound; the peer's outbound (your inbound, read-only to you) — named per the filename grammar in the channel protocol |
